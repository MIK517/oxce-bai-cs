...
| Bases, facilities, storage, and transfers | compatible for logistics subset | `Target.cpp`, `Base.cpp`, `BaseFacility.cpp`, `Transfer.cpp` | `campaign-foundation`; `strategic-logistics`; mobile-save/critical-sale tests; private corpus | Base identity/coordinates, facility flags, stock, staff, incoming quantities/hours and nested soldier/craft payloads round-trip. Persistent IDs and preservation keys distinguish movement, deletion and recreation. Completed arrivals remove transfer ownership exactly once. Facility editing and project accounting remain later work. |
| Soldiers, crafts, and equipment | compatible for logistics subset | `Soldier.cpp`, `SoldierDiary.cpp`, `Craft.cpp`, `Vehicle.cpp`, `ItemContainer.cpp` | `strategic-logistics`; save-adapter/unit fixtures; private UFO/TFTD/Rosigma saves | Identity, scalar personal state, stats, armor, assignments, transformation/commendation state, craft status/fuel/damage, weapons, cargo and vehicle dimensions are owned. Unknown diary/equipment/script fields remain in mobile source overlays. Newly generated state is persisted before transit and cannot reroll during reload. |
