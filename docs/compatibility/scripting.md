...
|---|---|---|---|---|
| Whitespace, line tracking, and `#` comments | not started | `src/Engine/Script.cpp` (`ScriptRefTokens::getNextToken`) | `script-core` (`comment`) | Ignore comments through newline and retain one-based line positions in diagnostics. Add focused whitespace/newline cases. |
| Symbols, dotted references, and labels | not started | `src/Engine/Script.cpp` (`ScriptRefTokens`, `findOperationAndArg`) |  | Split symbol characters, dotted receiver lookup, colon labels, duplicate/unknown labels, and segmented names into individual rows. |
| Statement terminator and token errors | not started | `src/Engine/Script.cpp` (`ScriptRefTokens::getNextToken`, `ScriptParserBase::parseBase`) | `script-core` (`invalid-number`) | Semicolon is mandatory; invalid tokens identify the argument and one-based line. Add missing-semicolon and invalid punctuation cases. |
| Decimal, hexadecimal, binary, and octal integers | not started | `src/Engine/Script.cpp` (`SelectedToken::parse`, numeric token parsing) | `script-core` (`locals-and-arithmetic`, `comment`, `invalid-number`) | Preserve signed 32-bit conversion, prefix rules, overflow rejection/wrapping behavior, and malformed-literal diagnostics. |
| Text literals and escapes | not started | `src/Engine/Script.cpp` (`ScriptRefTokens::getNextToken`, `ScriptText`) |  | Inventory delimiters, escapes, embedded newlines, empty text, storage lifetime, and invalid forms. |
| Primitive `int`, `text`, `label`, and `null` types | not started | `src/Engine/Script.h` (`ArgEnum`, `ArgRegisteType`); `ScriptParserBase` constructor | `script-inventory` | Preserve base/specialized register, variable, pointer, and editable-pointer compatibility. `__` remains the argument-separator pseudo-type. |
| Registers, output registers, locals, and constants | not started | `src/Engine/Script.h` (`ScriptWorkerBase`, `ParserWriter`); `src/Engine/Script.cpp` (`parseVar`, `parseConst`) | `script-core` (`return-only`, `set-output`, `locals-and-arithmetic`) | Preserve declaration-before-operation rules, initialization, writability, shadowing, register layout/alignment, and output copy-in/copy-out. |
...
| Overload resolution and argument separators | not started | `src/Engine/Script.cpp` (`findBestOverloadProc`); `src/Engine/ScriptBind.h` |  | Match compatibility scoring, writable-argument restrictions, ambiguity, receiver rewriting, and `__` separator behavior. |
| Compile diagnostics | not started | `src/Engine/Script.cpp` (`ScriptParserBase::parseBase`) | `script-core` rejected cases | Normalize structured port diagnostics while retaining reference failure category, operation/argument, parent/parser identity, and source line. |
| Runtime errors and execution limits | not started | `src/Engine/Script.cpp` (`ScriptWorkerBase::executeBase`); `src/Engine/Script.h` limits | `script-inventory` | Reference limits begin at 9 outputs, 16 parsed arguments, and `64 * sizeof(void*)` register bytes. Define safe instruction, recursion/event, trace, and allocation limits before VM implementation. |
