...
| Runtime errors and execution limits | compatible for binding-free VM | `src/Engine/Script.cpp` (`ScriptWorkerBase::executeBase`); `src/Engine/Script.h` limits | `script-core` (`division-by-zero`); compiler/VM/event limit unit fixtures | Output, argument, register, compiled-instruction, execution-step, trace-entry, and event-execution bounds are enforced. Operation failure retains prior state; missing host providers and provider failures are distinct. Recursive calls remain with the exact catalog slice. |
...
| Binding and constant catalog | partial infrastructure | `src/Engine/ScriptBind.h`; `ScriptRegister` call sites | `script-inventory`; API/host unit fixtures; [API/events review](../phase-4-api-events-review.md) | Immutable declarations now carry stable IDs, source locations, parser groups, parameters, parsers, and constants. The audit separates 96 constant occurrences from 407 callable candidates; exact template-resolved declarations and generation remain the focused catalog branch. |
...
| Global scripts and tags | partial | `src/Mod/Mod.cpp` (`ModScriptGlobal`); `src/Engine/Script.h` (`ScriptGlobal`) | tag/value unit fixtures; `script-events` | Catalog constants are filtered by parser group. Typed tag/value-type registration preserves global `Tag.` name uniqueness, one-based indices, owner limits, and source provenance. Exact `RuleList`, mod load order, custom codecs, and catalog population remain. |
...
| Event scripts | compatible for scalar event composition | `src/Engine/Script.h` (`ScriptParserEventsBase`, worker event execution); `src/Engine/Script.cpp` (`loadEvents`, `releseEvents`) | `script-events`; event unit fixtures | Stable offset order, negative/current/positive execution, `new`/`update`/`override`/`delete`/`ignore`, unknown-name severity, invalid offsets, and bounded execution match the pinned fixture. Read-only output reset and host argument registers await exact parser signatures. |
