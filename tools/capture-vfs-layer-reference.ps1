...
param(
    ...
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\fixtures\expected\vfs\layered-catalog.expected.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The VFS layer capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\vfs\vfs_layer_probe.cpp"
$fixture = Join-Path $repository "fixtures\public\vfs\layered-catalog.tsv"
...
$work = Join-Path $repository "artifacts\reference-vfs-layer"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "vfs_layer_probe.exe"
...
if ($LASTEXITCODE -ne 0) {
    throw "The C++ VFS layer probe failed to compile."
}

$raw = & $executable $fixture
if ($LASTEXITCODE -ne 0) {
    throw "The C++ VFS layer probe failed."
}

$rawPath = Join-Path $work "vfs-layer.raw.json"
...
if ($LASTEXITCODE -ne 0) {
    throw "The C++ VFS layer output could not be normalized."
}
