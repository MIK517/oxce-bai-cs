...
param(
    ...
    [string] $FixturePath,
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\fixtures\expected\images\indexed-gif.expected.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The indexed-GIF capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\images\indexed_gif_probe.cpp"
$fixture = if ([string]::IsNullOrWhiteSpace($FixturePath)) {
    Join-Path $repository "fixtures\public\images\indexed-gif.hex"
} else {
...
$work = Join-Path $repository "artifacts\reference-indexed-gif"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "indexed_gif_probe.exe"
...
if ($LASTEXITCODE -ne 0) { throw "The C++ indexed-GIF probe failed to compile." }
...
if ($LASTEXITCODE -ne 0) { throw "The C++ indexed-GIF probe failed." }
$rawPath = Join-Path $work "indexed-gif.raw.json"
...
if ($LASTEXITCODE -ne 0) { throw "The C++ indexed-GIF output could not be normalized." }
