...
param(
    ...
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\fixtures\expected\audio\cat-sound.expected.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The CAT sound capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\audio\cat_sound_probe.cpp"
$ufoFixture = Join-Path $repository "fixtures\public\audio\cat-sound-ufo-entry.hex"
$tftdFixture = Join-Path $repository "fixtures\public\audio\cat-sound-tftd-entry.hex"
...
$work = Join-Path $repository "artifacts\reference-cat-sound"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "cat_sound_probe.exe"
foreach ($value in @($devCommand, $probe, $ufoFixture, $tftdFixture, $dependencies, $executable, $work)) {
...
if ($LASTEXITCODE -ne 0) { throw "The C++ CAT sound probe failed to compile." }
Copy-Item -LiteralPath (Join-Path $dependencies "lib\x64\SDL.dll") -Destination $work -Force

$raw = & $executable $ufoFixture $tftdFixture
if ($LASTEXITCODE -ne 0) { throw "The C++ CAT sound probe failed." }
$rawPath = Join-Path $work "cat-sound.raw.json"
...
if ($LASTEXITCODE -ne 0) { throw "The C++ CAT sound output could not be normalized." }
