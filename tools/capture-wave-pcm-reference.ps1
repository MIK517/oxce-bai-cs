...
param(
    ...
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\fixtures\expected\audio\wave-pcm.expected.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The WAV PCM capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\audio\wave_pcm_probe.cpp"
$u8Fixture = Join-Path $repository "fixtures\public\audio\wave-pcm-u8.hex"
$s16Fixture = Join-Path $repository "fixtures\public\audio\wave-pcm-s16.hex"
...
$work = Join-Path $repository "artifacts\reference-wave-pcm"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "wave_pcm_probe.exe"
foreach ($value in @($devCommand, $probe, $u8Fixture, $s16Fixture, $dependencies, $executable, $work)) {
...
if ($LASTEXITCODE -ne 0) { throw "The C++ WAV PCM probe failed to compile." }
Copy-Item -LiteralPath (Join-Path $dependencies "lib\x64\SDL.dll") -Destination $work -Force

$raw = & $executable $u8Fixture $s16Fixture
if ($LASTEXITCODE -ne 0) { throw "The C++ WAV PCM probe failed." }
$rawPath = Join-Path $work "wave-pcm.raw.json"
...
if ($LASTEXITCODE -ne 0) { throw "The C++ WAV PCM output could not be normalized." }
