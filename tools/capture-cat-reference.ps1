...
param(
    ...
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\fixtures\expected\containers\cat-entries.expected.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The CAT capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\containers\cat_entries_probe.cpp"
$fixture = Join-Path $repository "fixtures\public\containers\cat-entries.hex"
...
$work = Join-Path $repository "artifacts\reference-cat"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "cat_entries_probe.exe"
...
if ($LASTEXITCODE -ne 0) { throw "The C++ CAT probe failed to compile." }

$raw = & $executable $fixture
if ($LASTEXITCODE -ne 0) { throw "The C++ CAT probe failed." }
$rawPath = Join-Path $work "cat-entries.raw.json"
...
if ($LASTEXITCODE -ne 0) { throw "The C++ CAT output could not be normalized." }
