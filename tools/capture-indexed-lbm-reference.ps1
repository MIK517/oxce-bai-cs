...
param(
    ...
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\fixtures\expected\images\indexed-lbm.expected.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The indexed-LBM capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\images\indexed_lbm_probe.cpp"
$pbmFixture = Join-Path $repository "fixtures\public\images\indexed-pbm.hex"
$ilbmFixture = Join-Path $repository "fixtures\public\images\indexed-ilbm.hex"
...
$work = Join-Path $repository "artifacts\reference-indexed-lbm"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "indexed_lbm_probe.exe"
foreach ($value in @($devCommand, $probe, $pbmFixture, $ilbmFixture, $dependencies, $executable, $work)) {
    if ($value -match '["&|<>^]') { throw "Reference capture paths contain an unsafe character." }
}

$compile = 'call "{0}" -no_logo -arch=x64 -host_arch=x64 && cd /d "{4}" && cl.exe /nologo /std:c++20 /EHsc /I"{3}\include\SDL" "{1}" /link /LIBPATH:"{3}\lib\x64" SDL.lib SDL_image.lib /OUT:"{2}"' -f `
    $devCommand, $probe, $executable, $dependencies, $work
& $env:ComSpec /d /c $compile
if ($LASTEXITCODE -ne 0) { throw "The C++ indexed-LBM probe failed to compile." }
Copy-Item -LiteralPath (Join-Path $dependencies "lib\x64\SDL.dll") -Destination $work -Force
Copy-Item -LiteralPath (Join-Path $dependencies "lib\x64\SDL_image.dll") -Destination $work -Force

$raw = & $executable $pbmFixture $ilbmFixture
if ($LASTEXITCODE -ne 0) { throw "The C++ indexed-LBM probe failed." }
$rawPath = Join-Path $work "indexed-lbm.raw.json"
...
if ($LASTEXITCODE -ne 0) { throw "The C++ indexed-LBM output could not be normalized." }
