...
param(
    ...
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\artifacts\reference-yaml-special-scalars\yaml-special-scalars.actual.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The YAML special-scalar capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\yaml\yaml_special_scalar_probe.cpp"
$fixture = Join-Path $repository "fixtures\public\yaml\special-scalar-conversions.yml"
...
$work = Join-Path $repository "artifacts\reference-yaml-special-scalars"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "yaml_special_scalar_probe.exe"
...
if ($LASTEXITCODE -ne 0) {
    throw "The C++ YAML special-scalar probe failed to compile."
}

$raw = & $executable $fixture
if ($LASTEXITCODE -ne 0) {
    throw "The C++ YAML special-scalar probe failed."
}

$rawPath = Join-Path $work "yaml-special-scalars.raw.json"
...
if ($LASTEXITCODE -ne 0) {
    throw "The C++ YAML special-scalar output could not be normalized."
}
