...
param(
    ...
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\fixtures\expected\audio\gm-cat.expected.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The GM.CAT capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\audio\gm_cat_probe.cpp"
$fixture = Join-Path $repository "fixtures\public\audio\gm-cat-entry.hex"
...
$work = Join-Path $repository "artifacts\reference-gm-cat"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "gm_cat_probe.exe"
...
if ($LASTEXITCODE -ne 0) { throw "The C++ GM.CAT probe failed to compile." }

$raw = & $executable $fixture
if ($LASTEXITCODE -ne 0) { throw "The C++ GM.CAT probe failed." }
$rawPath = Join-Path $work "gm-cat.raw.json"
...
if ($LASTEXITCODE -ne 0) { throw "The C++ GM.CAT output could not be normalized." }
