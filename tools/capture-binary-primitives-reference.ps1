...
param(
    ...
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\fixtures\expected\binary\endian-primitives.expected.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The binary primitive capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\binary\binary_primitive_probe.cpp"
$fixture = Join-Path $repository "fixtures\public\binary\endian-primitives.hex"
...
$work = Join-Path $repository "artifacts\reference-binary-primitives"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "binary_primitive_probe.exe"
...
if ($LASTEXITCODE -ne 0) {
    throw "The C++ binary primitive probe failed to compile."
}

$raw = & $executable $fixture
if ($LASTEXITCODE -ne 0) {
    throw "The C++ binary primitive probe failed."
}

$rawPath = Join-Path $work "binary-primitives.raw.json"
...
if ($LASTEXITCODE -ne 0) {
    throw "The C++ binary primitive output could not be normalized."
}
