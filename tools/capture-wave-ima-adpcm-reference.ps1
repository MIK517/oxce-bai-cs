...
param(
    ...
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\fixtures\expected\audio\wave-ima-adpcm.expected.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The WAV IMA ADPCM capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\audio\wave_ima_adpcm_probe.cpp"
$monoFixture = Join-Path $repository "fixtures\public\audio\wave-ima-adpcm-mono.hex"
$stereoFixture = Join-Path $repository "fixtures\public\audio\wave-ima-adpcm-stereo.hex"
...
$work = Join-Path $repository "artifacts\reference-wave-ima-adpcm"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "wave_ima_adpcm_probe.exe"
...
if ($LASTEXITCODE -ne 0) { throw "The C++ WAV IMA ADPCM probe failed to compile." }
...
if ($LASTEXITCODE -ne 0) { throw "The C++ WAV IMA ADPCM probe failed." }
$rawPath = Join-Path $work "wave-ima-adpcm.raw.json"
...
if ($LASTEXITCODE -ne 0) { throw "The C++ WAV IMA ADPCM output could not be normalized." }
