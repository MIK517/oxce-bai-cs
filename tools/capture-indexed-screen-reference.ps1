...
param(
    ...
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\fixtures\expected\images\indexed-screen-codecs.expected.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The indexed-screen capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\images\indexed_screen_probe.cpp"
$fixture = Join-Path $repository "fixtures\public\images\indexed-screen-codecs.hex"
...
$work = Join-Path $repository "artifacts\reference-indexed-screens"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "indexed_screen_probe.exe"
...
if ($LASTEXITCODE -ne 0) {
    throw "The C++ indexed-screen probe failed to compile."
}

$raw = & $executable $fixture
if ($LASTEXITCODE -ne 0) {
    throw "The C++ indexed-screen probe failed."
}

$rawPath = Join-Path $work "indexed-screen.raw.json"
...
if ($LASTEXITCODE -ne 0) {
    throw "The C++ indexed-screen output could not be normalized."
}
