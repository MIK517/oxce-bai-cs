...
param(
    ...
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\fixtures\expected\rendering\indexed-operations.expected.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The indexed-surface capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\rendering\indexed_operations_probe.cpp"
$fixture = Join-Path $repository "fixtures\public\rendering\indexed-operations.hex"
...
$work = Join-Path $repository "artifacts\reference-indexed-surface"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "indexed_operations_probe.exe"
...
$compile = 'call "{0}" -no_logo -arch=x64 -host_arch=x64 && cd /d "{4}" && cl.exe /nologo /std:c++20 /EHsc "{1}" /Fe:"{3}"' -f `
    $devCommand, $probe, $fixture, $executable, $work
& $env:ComSpec /d /c $compile
if ($LASTEXITCODE -ne 0) { throw "The C++ indexed-surface probe failed to compile." }

$raw = & $executable $fixture
if ($LASTEXITCODE -ne 0) { throw "The C++ indexed-surface probe failed." }
$rawPath = Join-Path $work "indexed-operations.raw.json"
...
if ($LASTEXITCODE -ne 0) { throw "The C++ indexed-surface output could not be normalized." }
