...
param(
    ...
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\fixtures\expected\images\pck-tab-sprites.expected.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The PCK/TAB capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\images\pck_tab_probe.cpp"
$fixture = Join-Path $repository "fixtures\public\images\pck-tab-sprites.hex"
...
$work = Join-Path $repository "artifacts\reference-pck-tab"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "pck_tab_probe.exe"
...
if ($LASTEXITCODE -ne 0) {
    throw "The C++ PCK/TAB probe failed to compile."
}

$raw = & $executable $fixture
if ($LASTEXITCODE -ne 0) {
    throw "The C++ PCK/TAB probe failed."
}

$rawPath = Join-Path $work "pck-tab.raw.json"
...
if ($LASTEXITCODE -ne 0) {
    throw "The C++ PCK/TAB output could not be normalized."
}
