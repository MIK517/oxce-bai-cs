...
param(
    ...
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\fixtures\expected\video\flc-indexed.expected.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The FLI/FLC capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\video\flc_indexed_probe.cpp"
$fixture = Join-Path $repository "fixtures\public\video\flc-indexed.hex"
...
$work = Join-Path $repository "artifacts\reference-flc"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "flc_indexed_probe.exe"
...
if ($LASTEXITCODE -ne 0) { throw "The C++ FLI/FLC probe failed to compile." }

$raw = & $executable $fixture
if ($LASTEXITCODE -ne 0) { throw "The C++ FLI/FLC probe failed." }
$rawPath = Join-Path $work "flc-indexed.raw.json"
...
if ($LASTEXITCODE -ne 0) { throw "The C++ FLI/FLC output could not be normalized." }
