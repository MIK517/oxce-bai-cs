...
param(
    ...
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\artifacts\reference-typed-rule-replay\typed-rule-replay.actual.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The typed-rule replay capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\mods\typed_rule_replay_probe.cpp"
$fixture = Join-Path $repository "fixtures\public\mods\typed-rule-replay\fixture\Ruleset\fixture.rul"
...
$work = Join-Path $repository "artifacts\reference-typed-rule-replay"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "typed_rule_replay_probe.exe"
...
if ($LASTEXITCODE -ne 0) {
    throw "The C++ typed-rule replay probe failed to compile."
}

$raw = & $executable $fixture
if ($LASTEXITCODE -ne 0) {
    throw "The C++ typed-rule replay probe failed."
}

$rawPath = Join-Path $work "typed-rule-replay.raw.json"
...
if ($LASTEXITCODE -ne 0) {
    throw "The C++ typed-rule replay output could not be normalized."
}
