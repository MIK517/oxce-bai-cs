...
param(
    ...
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\fixtures\expected\platform\input-audio.expected.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The input/audio capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\platform\input_audio_probe.cpp"
$fixture = Join-Path $repository "fixtures\public\platform\input-audio-semantics.tsv"
...
$work = Join-Path $repository "artifacts\reference-input-audio"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "input_audio_probe.exe"
...
if ($LASTEXITCODE -ne 0) { throw "The C++ input/audio probe failed to compile." }

$raw = & $executable $fixture
if ($LASTEXITCODE -ne 0) { throw "The C++ input/audio probe failed." }
$rawPath = Join-Path $work "input-audio.raw.json"
...
if ($LASTEXITCODE -ne 0) { throw "The C++ input/audio output could not be normalized." }
