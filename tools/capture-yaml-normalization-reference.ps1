...
param(
    ...
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\artifacts\reference-yaml-normalization\yaml-normalization.actual.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The YAML normalization capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\yaml\yaml_normalization_probe.cpp"
$fixture = Join-Path $repository "fixtures\public\yaml\representative-structures.yml"
...
$work = Join-Path $repository "artifacts\reference-yaml-normalization"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "yaml_normalization_probe.exe"
...
if ($LASTEXITCODE -ne 0) {
    throw "The C++ YAML normalization probe failed to compile."
}

$raw = & $executable $fixture
if ($LASTEXITCODE -ne 0) {
    throw "The C++ YAML normalization probe failed."
}

$rawPath = Join-Path $work "yaml-normalization.raw.json"
...
if ($LASTEXITCODE -ne 0) {
    throw "The C++ YAML normalization output could not be normalized."
}
