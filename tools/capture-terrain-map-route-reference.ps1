...
param(
    ...
    [string] $OutputPath = (Join-Path $PSScriptRoot "..\fixtures\expected\terrain\map-route.expected.json"),
    [string] $ExpectedCommit = "4df3a5e571a1a4b5e8a46d3161fb2e21a2adba15"
)

$ErrorActionPreference = "Stop"
if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
    throw "The terrain map/route capture currently requires Windows and Visual Studio C++ tools."
}
...
$probe = Join-Path $repository "fixtures\reference-probes\terrain\map_route_probe.cpp"
$mapFixture = Join-Path $repository "fixtures\public\terrain\map-layout.hex"
$routeFixture = Join-Path $repository "fixtures\public\terrain\route-nodes.hex"
...
$work = Join-Path $repository "artifacts\reference-terrain"
[IO.Directory]::CreateDirectory($work) | Out-Null
$executable = Join-Path $work "map_route_probe.exe"
foreach ($value in @($devCommand, $probe, $mapFixture, $routeFixture, $executable, $work)) {
...
$compile = 'call "{0}" -no_logo -arch=x64 -host_arch=x64 && cd /d "{5}" && cl.exe /nologo /std:c++20 /EHsc "{1}" /Fe:"{4}"' -f `
    $devCommand, $probe, $mapFixture, $routeFixture, $executable, $work
& $env:ComSpec /d /c $compile
if ($LASTEXITCODE -ne 0) { throw "The C++ terrain probe failed to compile." }

$raw = & $executable $mapFixture $routeFixture
if ($LASTEXITCODE -ne 0) { throw "The C++ terrain probe failed." }
$rawPath = Join-Path $work "map-route.raw.json"
...
if ($LASTEXITCODE -ne 0) { throw "The C++ terrain output could not be normalized." }
